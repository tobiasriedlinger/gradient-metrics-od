...
import src.model.frcnn_utils as det_utils
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # print(class_logits.shape, labels)
...
class RoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels, cand_filter=False):
        matched_idxs = []
        labels = []
    ...
    def select_training_samples(self,
        ...
        matched_idxs, labels = self.assign_targets_to_proposals(proposals, gt_boxes, gt_labels, cand_filter=cand_filter)
    ...
    def postprocess_detections(self,
                               ...
                               nms=True,        # type: Bool
                               ranking_by=None  # type: List[Tensor]
                               ):
        ...
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            ...
            if ranking_by is not None:
                rank = ranking_by[0].unsqueeze(-1).expand(-1, num_classes-1).reshape(-1)
                rank = rank[inds]
            ...
            if nms:
                if ranking_by is not None:
                    keep = box_ops.batched_nms(boxes, rank, labels, self.nms_thresh)
                else:
