...
from src.model.frcnn_utils import overwrite_eps
...
class FastRCNNPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.flatten(start_dim=1)
        x = func.dropout(x, training=True, p=self.dropout)
...
def fasterrcnn_resnet50_fpn(pretrained=False, progress=True,
    ...
    losses for both the RPN and the R-CNN.
    During inference, the model requires only the input tensors, and returns the post-processed
    predictions as a ``Lfasterrcnn_resnet50_fpnist[Dict[Tensor]]``, one for each input image. The fields of the ``Dict`` are as
    follows:
        - boxes (``FloatTensor[N, 4]``): the predicted boxes in ``[x1, y1, x2, y2]`` format, with
          ``0 <= x1 < x2 <= W`` and ``0 <= y1 < y2 <= H``.
        - labels (``Int64Tensor[N]``): the predicted labels for each image
