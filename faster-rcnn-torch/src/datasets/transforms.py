...
from torchvision.transforms import functional as trans

EPSILON = 1E-9


def default_transform_fn(img_size):
    return ComposeWithLabel([tv_tf.ToTensor()])


def random_transform_fn_xr(img_size):
    # Extreme ratio (referring to image aspect ratio)
    return ComposeWithLabel([RandomAdjustImage(),
                             RandomHorizontalFlipWithLabel(),
                             ClampLabel(),
                             tv_tf.ToTensor()])


def random_transform_fn_mr(img_size):
    # Moderate ratio
    return ComposeWithLabel([RandomAdjustImage(),
                             RandomAffineWithLabel(degrees=(-3, 3), shear=(-5, 5)),
                             PadToSquareWithLabel(fill=(127, 127, 127)),
                             RandomResizedCropWithLabel(img_size, scale=(0.9, 1.)),
...
class NormalizeWithLabel(tv_tf.Normalize):

    def __init__(self, mean=[0., 0., 0.], std=[1., 1., 1.]):
        super(NormalizeWithLabel, self).__init__(mean=mean, std=std)

    def __call__(self, img, label=None):
        img = super(NormalizeWithLabel, self).__call__(img)
...
class RandomResizedCropWithLabel(tv_tf.RandomResizedCrop):
    ...
    def __call__(self, img, label=None):
        ...
        img_tf = trans.resized_crop(img, i, j, h, w, self.size, self.interpolation)
...
class RandomRotationWithLabel(tv_tf.RandomRotation):
    ...
    def __call__(self, img: Image.Image, label=None):
        ...
        return trans.rotate(img, angle, self.resample, self.expand, self.center)
...
class RandomAffineWithLabel(tv_tf.RandomAffine):
    ...
    def __call__(self, img: Image.Image, label=None):
        ...
        shear = shear[0]
        img = trans.affine(img, rot_angle, translate, scale, shear, resample=self.resample, fillcolor=self.fillcolor)
...
class RandomAdjustImage(object):
    ...
    def __call__(self, img):
        ...
        img = trans.adjust_brightness(img, b)
        img = trans.adjust_contrast(img, c)
        img = trans.adjust_gamma(img, g)
        img = trans.adjust_hue(img, h)
        img = trans.adjust_saturation(img, s)
        return img
...
class PadToSquareWithLabel(object):
    ...
    def __call__(self, img, label=None):
        ...
        img = trans.pad(img, padding, self.fill, self.padding_mode)
        if label is None:
            return img, label
        label[..., 0] += padding[0]
        label[..., 1] += padding[1]
