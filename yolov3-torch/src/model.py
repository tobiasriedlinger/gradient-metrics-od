...
from src.config import ANCHORS, NUM_ANCHORS_PER_SCALE
...
class YoloLayer(nn.Module):
    ...
    def __init__(self, scale, stride, cfg):
    ...
    def forward(self, x):
        ...
        cls_pred = torch.sigmoid(prediction_raw[..., 5:]).view(num_batch, -1, self.cfg.num_classes)  # Cls pred one-hot.

        output = torch.cat((bbox_pred, conf_pred, cls_pred), -1)

        if self.training:
            return prediction_raw.view(num_batch, NUM_ANCHORS_PER_SCALE, -1, self.cfg.num_attrib), output
        else:
            return output
...
class DetectionBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale, stride, cfg):
        ...
        self.conv7 = nn.Conv2d(out_channels, cfg.last_layer_dim, 1, bias=True)
        self.yolo = YoloLayer(scale, stride, cfg)
        self.branch = None
    ...
    def forward(self, x):
        ...
        tmp = nn.functional.dropout(tmp, training=True, p=self.dropout_rate)
        tmp = self.conv7(tmp)
        out = self.yolo(tmp)
...
class DarkNet53BackBone(nn.Module):
    """
    Backbone of the Darknet53 classifier network containing 52 layers.
    """
...
class YoloNetTail(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.detect1 = DetectionBlock(1024, 1024, 'l', 32, cfg)
        self.conv1 = ConvLayer(512, 256, 1)
        self.detect2 = DetectionBlock(768, 512, 'm', 16, cfg)
        self.conv2 = ConvLayer(256, 128, 1)
        self.detect3 = DetectionBlock(384, 256, 's', 8, cfg)
...
class YoloNetV3(nn.Module):
    ...
    def __init__(self, cfg=YOLOV3Config(), post=True):
        super(YoloNetV3, self).__init__()
        self.darknet = DarkNet53BackBone()
        self.yolo_tail = YoloNetTail(cfg)
    ...
    def yolo_last_two_layers(self):
        _layers = self.yolo_last_layers() + [self.yolo_tail.detect1.conv6,
                                             self.yolo_tail.detect2.conv6,
                                             self.yolo_tail.detect3.conv6]
        return _layers

    def yolo_last_three_layers(self):
        _layers = self.yolo_last_two_layers() + [self.yolo_tail.detect1.conv5,
